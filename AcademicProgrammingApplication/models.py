...
class Semester(models.Model):
    #ATTRIBUTES
    period = models.CharField(primary_key=True, max_length=10, null=False, blank=False, verbose_name="Cohorte del semestre")
...
class Subject(models.Model):
    ...
    modality = models.CharField(max_length=200, choices=MODALITY_CHOICES, null=False, blank=False, verbose_name="Modalidad de la materia")
...
class Program(models.Model):
    ...
    cost = models.DecimalField(max_digits=10, decimal_places=2, null=False, blank=False, verbose_name="Costo de matricula")
    TYPE_CHOICES = [
        ('ESPECIALIZACION', 'Especialización'),
        ('MAESTRIA', 'Maestría'),
        ('DOCTORADO', 'Doctorado'),
    ]
    type = models.CharField(max_length=200, choices=TYPE_CHOICES, null=False, blank=False, verbose_name="Tipo de programa")
    MODALITY_CHOICES = [
        ('PRESENCIAL', 'Presencial'),
        ('VIRTUAL', 'Virtual'),
    ]
    modality = models.CharField(max_length=200, choices=MODALITY_CHOICES, null=False, blank=False, verbose_name="Modalidad del programa")
    ...
    # RELATIONS
    semesters = models.ManyToManyField(Semester, verbose_name="Semestres asociados") # A program can be associated with many semesters and a semester can be related to many programs
    subjects = models.ManyToManyField(Subject, verbose_name="Materias de programas") # A program can be associated with many subjects and a subject can be related to many programs
...
class Teacher(models.Model):
    #ATTRIBUTES
    id = models.CharField(primary_key=True, max_length=200, null=False, blank=False, verbose_name="Número de identificación")
...
class Class(models.Model):
    ...
    modality = models.CharField(max_length=200, choices=MODALITY_CHOICES, null=False, blank=False, verbose_name="Modalidad de la clase")
    ...
    # RELATIONS
    subject = models.ForeignKey(Subject, on_delete=models.CASCADE, verbose_name="Materia asociada") # A subject is related to many classes, but a class only to one subject
    teacher = models.ForeignKey(Teacher, on_delete=models.CASCADE, verbose_name="Profesor asociado") # A teacher can teach many classes, but a class can only have one teacher
...
class Contract(models.Model):
    ...
    contract_status = models.CharField(max_length=200, null=False, blank=False, choices=CONTRACT_STATUS_CHOICES, verbose_name="Estado del contrato")
    contact_preparation_date = models.DateField(null=False, blank=False, verbose_name="Fecha de elaboración")

    # RELATIONS
    id_teacher = models.OneToOneField(Teacher, on_delete=models.CASCADE, primary_key=True, null=False, blank=False, verbose_name="Profesor asociado") # A teacher must have a contract and a contract must be associated with a teacher
...
class Viatic(models.Model):
    ...
    # RELATIONS
    id_teacher = models.OneToOneField(Teacher, on_delete=models.CASCADE, primary_key=True, null=False, blank=False, verbose_name="Profesor asociado") # A teacher can have per diem associated with it and a per diem must be associated with a teacher
